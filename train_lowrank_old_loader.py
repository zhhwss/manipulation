...
from lhx.piper_real_cnn.data_loader import create_data_loaders
from model.model import RobotGraspModel_v2, RobotGraspModel_v3
from model.model3 import RobotGraspModelLowRank
...
def train_epoch(model, train_loader, criterion, optimizer, device):
    ...
    for frames, qpos, targets in tqdm(train_loader):
...
def validate(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for frames, qpos, targets in val_loader:
            frames = frames.to(device)
            qpos = qpos.to(device)
            targets = targets.to(device)
...
def main():
    ...
    sample_frames, sample_qpos, sample_outputs = next(iter(train_loader))

...
from model.model3 import RobotGraspModelLowRank, RobotGraspModelLowRankSVD
...
def determine_image_size(data_dir, image_type, scale=1.0, third_view=False):
...
def main():
    ...
    output_dir = os.path.join('result', f"low_rank_{timestamp}")
    ...
    image_shape = determine_image_size(
        args.data_dir, args.image_type, args.image_scale, args.third_view
    )
    ...
    # Compute SVD basis vectors if using SVD model
    svd_basis_vectors = None
    cumulative_variance = None
    if args.model_type == 'svd':
        svd_basis_vectors, mean_actions, singular_values, cumulative_variance = compute_svd_basis_vectors(
            full_dataset, rank=args.rank
        )
        
        # Save SVD results
        np.savez(os.path.join(output_dir, 'svd_results.npz'),
                 basis_vectors=svd_basis_vectors,
                 mean_actions=mean_actions,
                 singular_values=singular_values,
                 cumulative_variance=cumulative_variance)
    
    # Create model based on model_type
    if args.model_type == 'trainable':
        ...
        print(f"Using trainable low-rank model with {args.rank} basis vectors")
        
    elif args.model_type == 'svd':
        model = RobotGraspModelLowRankSVD(
            image_shape=image_shape,
            embed_dim=args.embed_dim,
            window_size=args.window_size,
            qpos_dim=qpos_dim,
            ...
            action_chunk=args.action_chunk,
            rank=args.rank,
            svd_basis_vectors=svd_basis_vectors
        ).to(device)
        print(f"Using SVD-based model with {args.rank} basis vectors (NOT trainable)")
        print(f"SVD explains {cumulative_variance[args.rank-1]:.4f} of variance")
        ...
        model = RobotGraspModelLowRankSVD(
            ...
            action_chunk=args.action_chunk,
            rank=args.rank,
            svd_basis_vectors=None  # This will make basis_vectors trainable
        ).to(device)
        print(f"Using SVD model architecture with {args.rank} trainable basis vectors")

...
def main():
    ...
    output_dir = os.path.join('result', f"multimodal_{timestamp}")
    ...
    # Validate that image shape is compatible with multimodal model
    if image_shape[0] != 4:
        print(f"Warning: MultiModalRobotGraspModel expects 4 channels (3 for point cloud + 1 for grayscale)")
        print(f"Current image shape: {image_shape}")
        print(f"Please ensure your data has 4 channels or modify the model accordingly.")
    ...
    print(f"  - Point cloud channels: {image_shape[0] if image_shape[0] >= 3 else 'N/A'}")
    print(f"  - Grayscale channel: {'Yes' if image_shape[0] >= 4 else 'No'}")

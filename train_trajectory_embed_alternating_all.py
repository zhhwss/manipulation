...
def update_trajectory_embeddings_all(model, val_loader, embed_optimizer, device, args):
...
def update_trajectory_embeddings_all(model, train_loader, val_loader, embed_optimizer, device, args):
    """Update trajectory embeddings using validation data"""
    model.eval()
    ...
    for indices, frames, qpos, targets in tqdm(train_loader, desc="train Updating Embeddings"):
        indices = indices.to(device)
        frames = frames.to(device)
        qpos = qpos.to(device)
        targets = targets.to(device)
    ...
    for indices, frames, qpos, targets in tqdm(val_loader, desc="val Updating Embeddings"):
        indices = indices.to(device)
        frames = frames.to(device)
        qpos = qpos.to(device)
        targets = targets.to(device)
...
def parse_args():
    ...
    parser.add_argument('--learning_rate', type=float, default=1e-4,
                       help='Learning rate')
    parser.add_argument('--embed_learning_rate', type=float, default=1e-4,
...
def main():
    ...
    output_dir = os.path.join('result', f"trajectory_color_embed_alternating_all_{timestamp}")
    ...
    # Determine image size
    image_shape = determine_image_size(args.data_dir, args.image_type, args.image_scale, args.third_view, args.color_view)
    ...
    args.num_trajectories = max_traj_idx + 1
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) % args.alternating_freq == 0:
            embed_loss, trajectory_losses = update_trajectory_embeddings_all(
                model, train_loader, val_loader, embed_optimizer, device, args
            )
